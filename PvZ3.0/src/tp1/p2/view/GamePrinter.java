...
public class GamePrinter {
	...
	protected String getInfo() {
		...
		/* @formatter:off */
		buffer.append(Messages.NUMBER_OF_CYCLES + SPACE + this.game.getCycle() + NEW_LINE);
		buffer.append(Messages.NUMBER_OF_COINS + SPACE + this.game.getSuncoins()+ NEW_LINE);
		buffer.append(Messages.REMAINING_ZOMBIES + SPACE + this.game.getRemainingZombies()+ NEW_LINE);
		buffer.append(Messages.GENERATED_SUNS + SPACE+ this.game.getGeneratedSuns() + NEW_LINE);
		buffer.append(Messages.CAUGHT_SUNS + SPACE+ this.game.getCaughtSuns() + NEW_LINE);
		buffer.append(Messages.SCORE + SPACE+ this.game.getScore() + NEW_LINE);
		/* @formatter:on */
	...
	public String endMessage() {
		...
		sb.append("\n");
		if(this.game.doesPlayerWin()) {
			sb.append(Messages.PLAYER_WINS + "\n");
		}else if (this.game.doZombiesWin()) {
			sb.append(Messages.ZOMBIES_WIN + "\n");
		}else {
			sb.append(Messages.PLAYER_QUITS + "\n");
		}
		if(this.game.thereIsANewRecord()) {
			sb.append(Messages.NEW_RECORD + this.game.getNewCurrentRecord());
		}
		return sb.toString();
	}
}
